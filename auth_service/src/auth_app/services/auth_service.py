...
class AuthService:
    ...
    async def sign_up(self, email: EmailStr, password: str) -> bool:
        ...
        user = await self.user_service.create(email, password)

        self.logger.info("Sign up user", email=user.email, user_id=user.id)
        return True
    ...
    async def sign_in(
        ...
        if not self.hash_service.verify(password, user.hashed_password):
            self.logger.warning("Sign in failed", email=user.email, user_id=user.id)
            raise InvalidEmailOrPassword
        ...
        self.logger.info("Sign in", email=user.email, user_id=user.id)
        return jwt.tokens
    ...
    async def update(
        ...
        self.logger.info("Update", old_email=old_email, new_email=new_email, id=user.id)
        return True
    ...
    async def refresh(self, refresh_token: str) -> JWTPair:
        ...
        self.logger.info("Refresh access", email=content["email"], id=content["sub"])
    ...
    async def get_login_history(
        ...
        self.logger.info("Get", user_id=user_id)
        return history
